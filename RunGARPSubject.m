...
flipLR = [zeros(1,ceil(allTrialsLength/2)), ones(1,floor(allTrialsLength/2))]; 
flipLR = flipLR(randperm(allTrialsLength));
...
%% All of this is just of outputting the reward
        if block == 1 && trialIndex == rewardTrial;
            left.Item  = allTrialsArray{1};
            right.Item = allTrialsArray{2};
        end

        if behavioral.key(trialIndex) == 'f';
            if flipLR(trialIndex) == 0; %%if the display was flipped, this conditional flips the reward
...
%%STILL IN WHILL LOOP
    ...
    if mod(trialOrder,blockLength) == 0; %This throws up the "break" screen between trials.
        drawBreak(w);
        Screen('Flip',w);
        WaitSecs(20);
        if input == 'k';
